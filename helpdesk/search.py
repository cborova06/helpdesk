...
# Compatibility shim: some hooks call helpdesk.search.remove_doc(doc)
# Provide a module-level function that delegates to HelpdeskSearch.remove_doc
def remove_doc(doc, method=None):
    try:
        HelpdeskSearch().remove_doc(doc)
    except Exception:
        frappe.log_error(title="helpdesk.search.remove_doc failed", message=frappe.get_traceback())
